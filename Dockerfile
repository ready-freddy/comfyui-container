...
RUN --mount=type=cache,target=/var/cache/apt --mount=type=cache,target=/var/lib/apt/lists \
    ...
    python3 python3-dev python3-venv python3-pip libglib2.0-0 libglib2.0-dev libgl1-mesa-dev \
    libxext6 libsm6 libopencv-core-dev libopencv-imgproc-dev libopencv-highgui-dev \
    libopencv-videoio-dev libopenblas-dev libomp-dev openssl unzip xz-utils; \
    rm -rf /var/lib/apt/lists/*

ARG NODE_VERSION=20.18.0
RUN set -eux; cd /tmp; arch=$(uname -m); case "$arch" in x86_64) A=x64;; aarch64) A=arm64;; *) exit 1;; esac; \
    ...
    ln -sf /usr/local/lib/nodejs/node-v${NODE_VERSION}-linux-${A}/bin/{node,npm,npx} /usr/local/bin/; node -v && npm -v

ARG CODE_SERVER_VERSION=4.92.2
RUN set -eux; arch=$(uname -m); case "$arch" in x86_64) CS=amd64;; aarch64) CS=arm64;; *) exit 1;; esac; \
    ...
    apt-get update; apt-get install -y /tmp/code-server.deb; rm -rf /var/lib/apt/lists/* /tmp/code-server.deb
...
RUN set -eux; python3 -m venv /opt/venvs/comfyui-perf; \
    ...
    pip install onnx onnxruntime-gpu==${ORT_VERSION} opencv-python-headless==${OPENCV_VERSION} fastapi uvicorn pydantic tqdm pillow
